...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_cls=dict(
            type=FocalLoss, # Focal Loss 处理类别不均衡
            use_sigmoid=True,
            gamma=2.0,
            alpha=0.25,
            loss_weight=1.0),  # 2.0 in DeformDETR
        loss_bbox=dict(type=L1Loss, loss_weight=5.0), # L1 Loss 计算检测框的坐标损失
        loss_iou=dict(type=GIoULoss, loss_weight=2.0)), # GIoU Loss 计算检测框的重叠误差
